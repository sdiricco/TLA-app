...
<template>
  ...
  <div class="mx-auto flex w-full max-w-screen-2xl flex-col gap-4 text-(--color-text) sm:gap-6">
    <!-- Section: Header -->
    <header><Button label="Tutti i giocatori" icon="pi pi-arrow-left" text severity="secondary" @click="router.push({ name: 'players' })" /><p class="mb-2 hidden text-xs font-extrabold tracking-[0.16em] text-primary sm:block">SCHEDA ATLETA</p><h1 class="text-3xl font-bold tracking-tight sm:text-5xl">Profilo giocatore</h1><p class="mt-3 hidden text-(--color-text-muted) sm:block">Identità, contatti e informazioni sportive.</p></header>
    ...
    <div v-else-if="!player" class="flex min-h-75 flex-col items-center justify-center border border-dashed border-(--color-border) bg-(--color-surface-soft) text-center"><span class="grid size-14 place-items-center rounded-full bg-primary-50 text-xl text-primary"><i class="pi pi-user-minus" /></span><h3 class="mt-4 text-xl font-bold">Giocatore non trovato</h3><p class="mt-2 text-sm text-(--color-text-muted)">Il profilo richiesto non è disponibile.</p></div>

    <div v-else class="grid gap-4 lg:grid-cols-[minmax(0,1.25fr)_minmax(20rem,0.75fr)]">
      <!------------------------------>
      <!-- Section: Player hero -->
      <!------------------------------>
      <section class="grid items-center gap-4 overflow-hidden border border-(--color-border) bg-primary-800 p-4 text-white sm:grid-cols-[auto_1fr] sm:p-8">
        <Avatar :label="getInitials(player.name)" :image="player.photo_url ?? undefined" shape="circle" class="size-20! sm:size-36!" />
      ...
      <!------------------------------>
      <section class="border border-(--color-border) bg-(--color-surface-card) p-4 sm:p-5">
        <header class="mb-4 flex flex-col items-stretch justify-between gap-3 sm:flex-row sm:items-center"><div class="flex items-center gap-3"><span class="grid size-10 shrink-0 place-items-center bg-primary-50 text-primary"><i class="pi pi-id-card" /></span><div><h2 class="font-bold">Informazioni personali</h2><p class="mt-1 text-xs text-(--color-text-subtle)">Dati anagrafici e contatti</p></div></div><Button v-if="auth.isAdmin" label="Modifica profilo" icon="pi pi-pencil" severity="secondary" outlined @click="router.push({ name: 'player-edit', params: { id: player.id } })" /></header>
      ...
      <!------------------------------>
      <section class="border border-(--color-border) bg-(--color-surface-card) p-4 lg:col-span-2 sm:p-5">
        <header class="mb-4 flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center"><div class="flex items-center gap-3"><span class="grid size-10 shrink-0 place-items-center bg-primary-50 text-primary"><i class="pi pi-chart-line" /></span><div><h2 class="font-bold">Risultati e statistiche</h2><p class="mt-1 text-xs text-(--color-text-subtle)">Andamento nelle partite completate</p></div></div><div v-if="matchHistory.recent_form.length" class="flex gap-1" aria-label="Forma recente"><span v-for="(outcome, index) in matchHistory.recent_form" :key="index" class="grid size-7 place-items-center text-xs font-extrabold" :class="outcome === 'win' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'">{{ outcome === 'win' ? 'V' : 'S' }}</span></div></header>
