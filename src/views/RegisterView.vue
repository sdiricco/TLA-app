...
<template>
  ...
  <main class="grid min-h-svh place-items-center bg-(--color-surface-soft) text-(--color-text) md:p-8 xl:p-10">
    <section class="grid min-h-svh w-full max-w-6xl overflow-hidden bg-(--color-surface-card) md:min-h-[calc(100svh-4rem)] md:grid-cols-2 md:rounded-lg md:border md:border-(--color-border) xl:min-h-[calc(100svh-5rem)]" aria-label="Registrazione a TLA App">
      ...
      <section class="flex min-h-svh flex-col justify-start px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-[max(1.5rem,env(safe-area-inset-top))] sm:px-10 md:min-h-0 md:justify-center md:p-10 xl:p-16">
        ...
        <div class="mx-auto w-full max-w-sm">
          ...
          <header class="mb-6">
            <p class="mb-2 text-xs font-extrabold tracking-[0.16em] text-primary">UNISCITI AL CLUB</p>
          ...
          <section v-if="auth.registrationPending" class="flex flex-col items-center gap-3 border border-(--color-border) bg-(--color-surface-soft) p-5 text-center">
            ...
            <span class="grid size-14 place-items-center rounded-full bg-primary-100 text-xl text-primary"><i class="pi pi-envelope" /></span>
          ...
          <form v-else class="flex flex-col gap-3.5" @submit.prevent="handleSubmit">
            ...
            <label for="password" class="grid gap-2 text-sm font-bold text-(--color-text-muted)">Password<Password id="password" v-model="password" placeholder="Crea una password" :feedback="true" autocomplete="new-password" fluid toggle-mask required /></label>
            <label for="confirmPassword" class="grid gap-2 text-sm font-bold text-(--color-text-muted)">Conferma password<Password id="confirmPassword" v-model="confirmPassword" placeholder="Ripeti la password" :feedback="false" autocomplete="new-password" fluid toggle-mask required /></label>

            <Button type="submit" label="Crea account" icon="pi pi-arrow-right" icon-pos="right" :loading="auth.loading" fluid />
