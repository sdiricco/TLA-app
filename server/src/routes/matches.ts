...
matchesRouter.patch('/:id/assign', requireAdmin, async (req, res) => {
  ...
  try {
    const existing = await prisma.match.findFirst({
      where: { id: matchId, tournament: { organizationId } },
      select: { id: true },
    })
    if (!existing) throw new Error('NOT_FOUND')
    if (player_id) {
      const playerExists = await prisma.player.count({ where: { id: player_id, organizationId } })
      if (!playerExists) {
...
matchesRouter.put('/:id', requireAdmin, async (req, res) => {
  ...
  try {
    const match = await prisma.$transaction(async (tx) => {
      ...
      if (!current || current.tournament.organizationId !== organizationId) throw new Error('NOT_FOUND')
